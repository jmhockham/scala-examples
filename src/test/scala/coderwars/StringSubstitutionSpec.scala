...
class StringSubstitutionSpec extends FlatSpec with Matchers {

  val msg = List[Int](1,2,3,7,3,2,3,7,2,3,4,8,9,7,8)
  val mapOfSubs = Map[Int, Char](23 -> 'N', 234 -> ' ', 89 -> 'H', 78 -> 'Q', 37 -> 'A')

  "subs" should "correctly replace a matching sub" in {
    StringSubstitution.subs(List(2,3), mapOfSubs) shouldBe "N"
    StringSubstitution.subs(List(1,2,3), mapOfSubs) shouldBe "1N"
  }

  "findSubInString" should "find a match if one exists" in {
    StringSubstitution.findSubInStr("abc23def",mapOfSubs) shouldBe Some((3,5,'N'))
  }
