...
integers X? For example, if X = {1, 3, 5}, you could climb 1, 3, or 5 steps at a time. Generalize your function
to take in X.

Apparently this is more like a fibo:
 ...
 */
object StepClimber extends App {

  def uniqueCombinationsForNumber(total: Int): Int = {
    if(total <= 1){
      1
    } else {
      uniqueCombinationsForNumber(total - 1) + uniqueCombinationsForNumber(total - 2)
    }
  }

  println(uniqueCombinationsForNumber(total = 4))
